<script lang="ts">
	...
	let { routes, width = 700, height = 500, originIatas = [] }: Props = $props();
	let hovered: string | null = $state(null);
	const dispatch = createEventDispatcher<{ hover: { iata: string | null } }>();
	...
	const sortedRoutes = $derived([...routes].sort((a, b) => a.total_cost - b.total_cost));
					...
					>
						<rect
							...
							on:mouseover={() => {
								...
								dispatch('hover', { iata: route.meeting_airport });
							}}
							on:mouseout={() => {
								hovered = null;
								// eslint-disable-next-line no-console
