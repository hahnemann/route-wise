...
/**
 * Returns the top 10 optimal meeting points sorted by lowest total cost.
 */
export function findTopMeetingPoints(
    graph: Graph,
    startPoints: AirportCode[],
    limit: number = 10
): { iata: AirportCode; totalCost: number }[] {
    // 1. Run Dijkstra for each traveler (same as your original logic)
    const travelDistances = startPoints.map(start => dijkstraToAll(graph, start));
    const results: { iata: AirportCode; totalCost: number }[] = [];

    // 2. Calculate total cost for every airport in the graph
    for (const airport in graph) {
        let totalCostForThisAirport = 0;
        let reachableByAll = true;
        ...
        if (reachableByAll) {
            results.push({ iata: airport, totalCost: totalCostForThisAirport });
        }
    }

    // 3. Sort by cost (ascending) and take the top N
    return results
